...
function Search() {
  ...
  function resultRows() {
    if(results?.data?.length) {
      ...
      return (
        <tr>
          <th scope="row" colSpan={3} className='text-center'>No results found.</th>
        </tr>
      );
    }
  }
    ...
    if(results?.data?.length) {
      return (
        <div className='d-flex justify-content-center'>
          <Pager currentPage={results.page} pageSize={results.pageSize} totalCount={results.totalCount} onChange={p => pageChanged(p)} />
        </div>
      );
    }
  }
